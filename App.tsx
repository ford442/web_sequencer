...
const App: React.FC = () => {
  ...
  const onStep = useCallback((step: number) => {
    ...
    // Play frozen parts on the first step of the loop
    if (step === 0) {
  ...
  const handlePatternChange = useCallback((part: keyof Pattern, stepIndex: number) => {
  ...
  const handleDrumParamsChange = useCallback((sound: DrumSound, newParams: KickParams | SnareParams | HatParams) => {
