...
## 2026-07-09 - [VoiceFXStrip Object Pooling Optimization]
**Learning:** In highly polyphonic Web Audio engines, dynamically allocating and connecting native nodes (BiquadFilterNode, StereoPannerNode, WaveShaperNode) per note trigger introduces measurable garbage collection pauses and main-thread overhead during dense playback. The attempt to optimize this with regex string-replacements failed catastrophically due to unmatched braces and TS structure corruption.
**Action:** Always use an AudioNodePool to pre-allocate reusable node wrappers (like `VoiceFXStrip`) and implement these optimizations using precise string replacement or structural diffing on known-good commits instead of blind regex.
