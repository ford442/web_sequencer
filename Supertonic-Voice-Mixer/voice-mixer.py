...
class LatentExplorerDSP(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.sl_speed.setRange(50, 200); self.sl_speed.setValue(105)
        self.lbl_speed = QLabel("1.05x")
        ...
        self.spin_steps.setRange(1, 50); self.spin_steps.setValue(5)
    ...
    def load_voice_json(self):
        fname, _ = QFileDialog.getOpenFileName(self, "Open Voice Style", "assets/voice_styles", "JSON Files (*.json)")
        if not fname:
            return
        try:
    ...
    def load_voice_library(self):
        fnames, _ = QFileDialog.getOpenFileNames(self, "Select Multiple Voices", "assets/voice_styles", "JSON Files (*.json)")
        if not fnames:
            return
    ...
    def save_voice_json(self):
        ...
        fname, _ = QFileDialog.getSaveFileName(self, "Save Voice Style", "assets/voice_styles", "JSON Files (*.json)")
        if not fname: return
        try:
