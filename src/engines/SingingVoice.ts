...
export class SingingVoice {
  ...
  setVibratoDepth(percent: number, time?: number): void {
    if (this.workletNode) {
      this.workletNode.parameters
        .get("vibratoDepth")
        ?.setValueAtTime(percent / 100, time || this.audioContext.currentTime);
    }
  }
  ...
  setVibratoRate(rate: number, time?: number): void {
    if (this.workletNode) {
      this.workletNode.parameters
        .get("vibratoRate")
        ?.setValueAtTime(rate, time || this.audioContext.currentTime);
    }
  }
  ...
  setBreathIntensity(intensity: number, time?: number): void {
    if (this.workletNode) {
      this.workletNode.parameters
        .get("breathIntensity")
        ?.setValueAtTime(intensity, time || this.audioContext.currentTime);
    }
  }
  ...
  setGrainPitchQuantize(semitones: number, time?: number): void {
    if (this.workletNode) {
      this.workletNode.parameters
        .get("grainPitchQuantize")
        ?.setValueAtTime(semitones, time || this.audioContext.currentTime);
    }
  }
  ...
  setAttack(attack: number, time?: number): void {
    ...
    if (this.workletNode) {
      this.workletNode.parameters
        .get("attack")
        ?.setValueAtTime(attack, time || this.audioContext.currentTime);
    }
  }
  ...
  setDecay(decay: number, time?: number): void {
    ...
    if (this.workletNode) {
      this.workletNode.parameters
        .get("decay")
        ?.setValueAtTime(decay, time || this.audioContext.currentTime);
    }
  }
  ...
  setSustain(sustain: number, time?: number): void {
    if (this.workletNode) {
      this.workletNode.parameters
        .get("sustain")
        ?.setValueAtTime(sustain, time || this.audioContext.currentTime);
    }
  }
  ...
  setRelease(release: number, time?: number): void {
    if (this.workletNode) {
      this.workletNode.parameters
        .get("release")
        ?.setValueAtTime(release, time || this.audioContext.currentTime);
    }
  }
  ...
  setDownsample(factor: number, time?: number): void {
    if (this.workletNode) {
      this.workletNode.parameters
        .get("downsample")
        ?.setValueAtTime(factor, time || this.audioContext.currentTime);
    }
  }
