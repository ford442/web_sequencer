...
import { hasOpen303Native } from './AudioDSP';
...
// URL of the main hyphon_native WASM binary for native Open303 support.
// This constant is the same path used by hyphon_native.js (generated by emscripten/build.sh).
...
export class Open303Oscillator {
    ...
    async init(audioContext: AudioContext, workletUrl?: string, config?: Open303Config): Promise<boolean> {
        ...
        if (audioContext.audioWorklet && workletUrl) {
            try {
                // Determine WASM source.
                // Prefer hyphon_native.wasm when the main module already exposes the
                // Open303 native API – this means a single binary covers everything.
                // Otherwise fall back to the standalone jc303-single.wasm artifact.
                const useNative = hasOpen303Native() && !cfg.forceSingleThreaded;
                const wasmUrl   = useNative ? HYPHON_NATIVE_WASM_URL : jc303WasmUrl;

                console.log(`[Open303Oscillator] Fetching WASM: ${wasmUrl} (native=${useNative})`);
                const wasmResponse = await fetch(wasmUrl);

                if (!wasmResponse.ok) {
                    // If hyphon_native fetch failed, try the jc303 fallback before giving up
                    if (useNative) {
                        console.warn(`[Open303] hyphon_native.wasm fetch failed (${wasmResponse.status}), retrying with jc303-single.wasm`);
                        const fallbackResponse = await fetch(jc303WasmUrl);
                        if (fallbackResponse.ok) {
                            const fallbackBytes = await fallbackResponse.arrayBuffer();
                            return this._initWithWasmBytes(audioContext, workletUrl, fallbackBytes, false);
                        }
                    }
                ...
                // is hundreds of KB; anything under 1 KB means the build hasn't run yet.
                // hyphon_native.wasm is always large (includes Rubberband etc.) so this
                // guard only fires for the jc303-single stub.
                if (!useNative && wasmBytes.byteLength < 1024) {
                ...
                return this._initWithWasmBytes(audioContext, workletUrl, wasmBytes, useNative);
