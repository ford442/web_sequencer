                ...
                {contextMenu && (
                    <div style={{ position: 'fixed', top: 0, left: 0, zIndex: 9999 }}>
                        <NoteSelector
                            ...
                            currentDrive={contextMenu.track === 'sampler' ? pattern.sampler[activeSamplerBank]?.steps[contextMenu.step]?.drive : (pattern as any)?.[contextMenu.track]?.steps?.[contextMenu.step]?.drive}
                            currentCharacterMorph={contextMenu.track === 'sampler' ? pattern.sampler[activeSamplerBank]?.steps[contextMenu.step]?.characterMorph : (pattern as any)?.[contextMenu.track]?.steps?.[contextMenu.step]?.characterMorph}
                            currentReverbSend={contextMenu.track === 'sampler' ? pattern.sampler[activeSamplerBank]?.steps[contextMenu.step]?.reverbSend : (pattern as any)?.[contextMenu.track]?.steps?.[contextMenu.step]?.reverbSend}
                            currentDelaySend={contextMenu.track === 'sampler' ? pattern.sampler[activeSamplerBank]?.steps[contextMenu.step]?.delaySend : (pattern as any)?.[contextMenu.track]?.steps?.[contextMenu.step]?.delaySend}
                            currentChoir={contextMenu.track === 'sampler' ? pattern.sampler[activeSamplerBank]?.steps[contextMenu.step]?.choir : (pattern as any)?.[contextMenu.track]?.steps?.[contextMenu.step]?.choir}
