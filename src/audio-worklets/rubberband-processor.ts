...
import { PhonemeToneFilter, SyllableVolumeFilter, TranceGate, TransientExtractor } from "./rubberband/toneFilters";
...
class RubberBandProcessor extends AudioWorkletProcessor {
  ...
  process(_inputs: Float32Array[][], outputs: Float32Array[][], parameters: Record<string, Float32Array>): boolean {
    ...
    const consonantCrispness = parameters.consonantCrispness ? parameters.consonantCrispness[0] : 0.0;
    ...
    try {
      ...
      if (availOutput > 0) {
        ...
        // Consonant Crispness - Boost transients during non-vowel phonemes
        if (consonantCrispness > 0) {
          const isVowelForCrispness = this.getPhonemeDataAtSample(this.currentSamplePtr)[7];
          const sRateForCrispness = resolveWorkletSampleRate({ sampleRate: this.sampleRate || globalThis.sampleRate });
          this.transientExtractor.process(outputs, consonantCrispness, isVowelForCrispness, sRateForCrispness);
        }
