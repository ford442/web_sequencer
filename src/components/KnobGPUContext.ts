...
// Preserved exactly from MagicKnob.tsx; shared across all knob instances.
const SHADER_CODE = `
    struct Uniforms {
      time: f32,
      value: f32,
...
class KnobGPUContextClass {
    ...
    register(canvas: HTMLCanvasElement, getValue: () => number): SlotHandle | null {
        ...
        this.ensureInit().then((success) => {
            if (!this.pendingIds.has(id)) return; // Unregistered before init completed
            this.pendingIds.delete(id);

            if (!success || !this.device || !this.pipeline || !this.format) return;
    ...
    private async doInit(): Promise<boolean> {
        ...
        try {
            ...
            // Silently skip if WebGPU is unavailable (canvas stays blank)
            return false;
        }
    }
    ...
    private renderAll(): void {
        ...
        // Tradeoff: A single command encoder can host multiple render passes targeting
        // different canvas textures. This lets us submit one command buffer for all knobs,
        // minimizing queue-submission overhead and keeping all knobs perfectly synchronized.
        // If a single pass throws (e.g. canvas context lost), we catch and skip that slot
        // so the rest of the batch still renders.
        ...
        for (const slot of this.slots.values()) {
            try {
                ...
                // Skip slots that fail (e.g. canvas not visible or context lost)
            }
        }
