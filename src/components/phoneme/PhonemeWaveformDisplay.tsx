...
export const PhonemeWaveformDisplay = memo(({ audioBuffer, width, height, phonemes, onLoadSample }: PhonemeWaveformDisplayProps) => {
  ...
  if (!audioBuffer) {
    return (
      <div role="status" aria-live="polite" className="flex flex-col items-center justify-center py-12 px-4 text-center bg-zinc-900/40 border border-dashed border-zinc-700 rounded-md mb-1 w-full relative group">
  ...
  return (
    <canvas
      ...
      role="img"
      aria-label="Phoneme Waveform Visualization"
    />
  );
});
