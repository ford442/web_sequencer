        ...
        expect(shader).toMatchInlineSnapshot(`
          "
              ...
              fn fs_main(in: VertexOutput) -> @location(0) vec4f {
                ...
                let ring_dist = abs(length(rot_uv) - 0.5500);
                ...
                if (arc_dist < 0.03 && pixel_angle >= -max_angle && pixel_angle <= val_mapped) {
                    ...
                    color = mix(vec3f(0.0000, 0.6000, 0.5000), vec3f(0.2000, 1.0000, 0.8000), arc_t);
                ...
                return vec4f(color * alpha * 1.5000, alpha);
              }
          "
        `);
    });
