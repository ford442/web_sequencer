...
export function useSamplerPanelState({
  ...
  const paramHandlers = useMemo(() => {
    const paramNames = [
      ...
      'sustain', 'release', 'choir', 'glitchChance', 'gateDepth', 'gateRate', 'reverbLfoRate', 'reverbLfoDepth', 'bitcrush', 'spectralComp', 'subHarmonics', 'vocalChorus', 'downsample', 'spectralCompression',
    ] as const;
