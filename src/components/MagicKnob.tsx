...
export const MagicKnob: React.FC<MagicKnobProps> = ({
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            const normalizedValue = Math.max(0, Math.min(1, (valueRef.current - min) / ((max - min) || 1)));
            ...
            device.queue.writeBuffer(uniformBuffer, 0, new Float32Array([now, normalizedValue, canvas.width, canvas.height]));
            pass.draw(3);
            pass.end();
    ...
    }, [device, preferredFormat, min, max]);
