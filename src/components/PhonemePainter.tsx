...
export const PhonemePainter: React.FC<PhonemePainterProps> = React.memo(
  ({ isOpen, onClose, stepIndex, note, audioBuffer, onSave, alignment }) => {
    ...
    useEffect(() => {
      if (isOpen && note) {
        if (note.phonemes && note.phonemes.length > 0) {
          ...
          const alignedPhonemes: PhonemeData[] = alignment.phonemes.map(
            (ph, idx) => ({
              id: generateId(),
              symbol: ph.phoneme,
    ...
    const handleDrag = useCallback((id: string, deltaX: number) => {
      setPhonemes((prev) => {
        ...
        const newStart = Math.max(
          0,
          Math.min(1 - duration, ph.start + deltaNormalized),
        );
        ...
        const handlePointerMove = (e: PointerEvent) => {
          ...
          setPhonemes((prev) => {
            ...
            if (resizeState.current!.side === "left") {
              const newStart = Math.max(
                0,
                Math.min(p.end - 0.05, startValue + deltaNormalized),
              );
              newPhonemes[idx] = { ...p, start: newStart };
            } else {
              const newEnd = Math.max(
                p.start + 0.05,
                Math.min(1, startValue + deltaNormalized),
              );
    ...
    const handlePitchBendChange = useCallback((id: string, bend: number) => {
      setPhonemes((prev) =>
        prev.map((p) => (p.id === id ? { ...p, pitchBend: bend } : p)),
      );
    }, []);
    ...
    return (
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(145deg, rgba(24,24,27,0.98), rgba(9,9,11,0.99))",
            borderColor: "rgba(6,182,212,0.25)",
            boxShadow:
              "0 0 60px rgba(6,182,212,0.15), 0 20px 60px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.05)",
          }}
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, rgba(6,182,212,0.1) 0%, transparent 50%, rgba(168,85,247,0.1) 100%)",
            }}
          />
          ...
          <div className="relative border-b border-cyan-500/20 px-6 py-4">
            ...
            <div
              ...
              style={{
                background:
                  "linear-gradient(90deg, transparent 0%, rgba(6,182,212,0.1) 30%, rgba(168,85,247,0.1) 70%, transparent 100%)",
              }}
            />
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 font-mono mt-0.5">
                    Step {stepIndex + 1} • {note?.note || "C4"} • Melodic
                    Sampler
                  </p>
                </div>
              </div>
          ...
          <div className="p-6 space-y-4">
            ...
            >
              ...
              {showAddMenu && (
                <div
                  ...
                  style={{
                    ...
                    boxShadow:
                      "0 12px 40px rgba(0,0,0,0.6), 0 0 20px rgba(6,182,212,0.1)",
                  }}
                >
                  ...
                  <div className="max-h-48 overflow-y-auto p-2 space-y-2">
                    {COMMON_PHONEMES.map((group) => (
                      <div key={group.cat} role="group" aria-label={group.cat}>
                        <div className="text-[9px] text-zinc-500 font-mono uppercase mb-1.5 px-1">
                          {group.cat}
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {group.phones.map((ph, idx) => (
                            <button
                              ...
                              key={ph}
                              ref={(el) => {
                                phonemeButtonRefs.current[idx] = el;
                              }}
              ...
              >
                ...
                {Array.from({ length: 9 }, (_, i) => (
                  ...
                  >
                    <span className="absolute -top-4 text-[9px] text-zinc-600 font-mono">
                      {i * 12.5}%
                    </span>
                  </div>
                ))}
            ...
            {selectedPhoneme && (
              ...
              >
                ...
                <div
                  ...
                  style={{
                    backgroundColor: getPhonemeColor(selectedPhoneme.symbol),
                  }}
                />
                <div className="flex items-center gap-6 flex-wrap relative z-10">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-xs text-zinc-400 font-mono">
                        {PHONEME_NAMES[selectedPhoneme.symbol.toUpperCase()] ||
                          selectedPhoneme.symbol}
                      </div>
                      <div className="text-[10px] text-zinc-600 font-mono">
                        {(
                          (selectedPhoneme.end - selectedPhoneme.start) *
                          100
                        ).toFixed(0)}
                        ms
                      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="relative w-32 h-6 bg-zinc-800 rounded-full overflow-hidden border border-zinc-700">
                      <div
                        ...
                        style={{
                          right:
                            selectedPhoneme.pitchBend >= 0
                              ? `${50 - (selectedPhoneme.pitchBend / 200) * 50}%`
                              : "50%",
                          left:
                            selectedPhoneme.pitchBend <= 0
                              ? `${50 + (selectedPhoneme.pitchBend / 200) * 50}%`
                      ...
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <span
                          className={`text-[10px] font-mono ${selectedPhoneme.pitchBend !== 0 ? "text-white" : "text-zinc-600"}`}
                        >
                          {selectedPhoneme.pitchBend > 0 ? "+" : ""}
                          {selectedPhoneme.pitchBend}¢
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-4 text-[10px] text-zinc-500 font-mono">
                    <span>
                      Start: {(selectedPhoneme.start * 100).toFixed(1)}%
                    </span>
            ...
            <div className="flex items-center gap-4 text-[10px] text-zinc-500 font-mono flex-wrap">
              ...
              <span className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-950/50 border border-zinc-800">
                <span className="w-2 h-2 rounded-sm bg-purple-500 shadow-[0_0_6px_rgba(168,85,247,0.6)]" />
                Vowel
              </span>
              <span className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-950/50 border border-zinc-800">
                <span className="w-2 h-2 rounded-sm bg-red-500 shadow-[0_0_6px_rgba(239,68,68,0.6)]" />
                Plosive
              </span>
              <span className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-950/50 border border-zinc-800">
                <span className="w-2 h-2 rounded-sm bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.6)]" />
                Fricative
              </span>
              <span className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-950/50 border border-zinc-800">
                <span className="w-2 h-2 rounded-sm bg-orange-500 shadow-[0_0_6px_rgba(249,115,22,0.6)]" />
                Affricate
              </span>
              <span className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-950/50 border border-zinc-800">
                <span className="w-2 h-2 rounded-sm bg-blue-500 shadow-[0_0_6px_rgba(59,130,246,0.6)]" />
                Nasal
              </span>
              <span className="flex items-center gap-1.5 px-2 py-1 rounded bg-zinc-950/50 border border-zinc-800">
