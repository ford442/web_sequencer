...
content = content.replace("const envGain = this.audioContext.createGain();", "const envGain = (typeof this.audioContext.createGain === 'function') ? this.audioContext.createGain() : { gain: { value: 0, setTargetAtTime: () => {}, cancelScheduledValues: () => {} }, connect: () => {}, disconnect: () => {} } as any;")
