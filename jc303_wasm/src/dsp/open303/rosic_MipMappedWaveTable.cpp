...
MipMappedWaveTable::MipMappedWaveTable()
{
  ...
  waveform   = 0;
  symmetry   = 0.5;
  initialized = false;  // WASM: track initialization state
  ...
  // WASM FIX: Don't auto-generate wavetables in constructor to avoid stack overflow
  // The wavetables will be generated on first setWaveform() call or explicitly
  // via ensureInitialized(). This prevents deep call chains during Open303 construction.
}
