...
## 2026-05-27 - Context-Rich Tooltips for Custom Selectors
**Learning:** Discovered that deep interactive elements like custom dropdown options (e.g., piano flyouts) often rely solely on visual cues (like color) to communicate state (e.g., 'in scale' vs 'out of scale'). Adding a robust  attribute dynamically generated from this state not only provides essential hover feedback for mouse users but also enriches screen reader context when mirrored with `aria-label`.
**Action:** When mapping arrays to render custom  buttons, actively inject state-derived string templates into both  and  attributes to ensure parity between visual indicators and accessible descriptions.
## 2024-05-15 - Context-Rich Tooltips for Custom Selectors
**Learning:** Discovered that deep interactive elements like custom dropdown options (e.g., piano flyouts) often rely solely on visual cues (like color) to communicate state (e.g., 'in scale' vs 'out of scale'). Adding a robust `title` attribute dynamically generated from this state not only provides essential hover feedback for mouse users but also enriches screen reader context when mirrored with `aria-label`.
**Action:** When mapping arrays to render custom `role="option"` buttons, actively inject state-derived string templates into both `title` and `aria-label` attributes to ensure parity between visual indicators and accessible descriptions.
