...
- Completed "Explore a 'breathing' noise gate for TTS that slightly increases noise floor gain during vowels and cuts it during silence for a vintage sampler feel." by updating `ExpressiveVoiceProcessor` to smoothly fade down breath noise for non-vowels and gap sections based on the shared `isVowel` flag from the `RubberBandProcessor`.
- Velocity Check: Utilizing the existing `isVowel` SAB property avoiding adding any new DSP AudioParams, and implementing a simple 1-pole smoother successfully eliminated clicks during consonant/vowel transitions while giving the breath noise a much more natural, vintage sampler feel.
